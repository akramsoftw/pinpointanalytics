        ...
        <div class="container container-sm">
            ...
            <div class="box-general diff-so shadow">
                <div class="heading">
                    <div class="row">
                        <div class="col-6">
                            <h3><a href="{{route('options_index')}}" class="back"><img src="{{ asset('frontend/img/pinpoint/icon/ic-left.svg')}}" class="img-fluid img-icon" alt="Icon"></a> Game</h3>
                        </div>
                        <div class="col-6">
                            <a href="{{route('game_create')}}" class="btn btn-sm btn-success float-right">Add Game</a>
                        </div>
                    </div>                    
                </div>
                    ...
                    <table class="table table-option">
                        ...
                        <tbody>
                            @foreach($games as $game)
                                <tr>
                                    <td>{{ \Carbon\Carbon::parse($game->game_time)->format('l, F d') }}</td>
                                    <td>{{$game->get_opponent_team->name}}</td>
                                    <td>{{$game->location}}</td>
                                    <td>
                                        @if($game->is_active != 1)
                                            <a href="{{route('enable_game', $game->id)}}" class="btn btn-sm btn-warning btn-action">Activate</a>
                                        @else
